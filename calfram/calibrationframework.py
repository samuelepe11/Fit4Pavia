...
def h_triangle(new_pts, tilde):
    ...
    for idx, points in enumerate(zip(new_pts, tilde)):
        if idx == 0:
            ...
            b = sorted_side[1]
            a = sorted_side[2]
            area = 1/4*np.sqrt(np.abs(((a+(b+c))*(c-(a-b))*(c+(a-b))*(a+(b-c)))))
            h = 2*area/ab
...
def classwise_calibration(measures):
    ...
    temp_underconf = [measures[key]['ec_underconf'] for key in measures.keys()]
    if np.all(np.isnan(temp_underconf)):
        classes_underconf = 1.0
    else:
        classes_underconf = np.nanmean(temp_underconf).round(3)
    if np.isnan(classes_underconf):
        print("Error in the computation of 'ec_underconf'")
        classes_underconf = 1.0
